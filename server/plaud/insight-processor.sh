...
set -u
# launchd에는 로케일이 없어 C 로케일로 돌면 문자열 처리가 바이트 단위가 됨
# (한글 슬러그가 중간에서 잘려 APFS가 거부하는 파일명이 생김 — 2026-07-17 실사용에서 발생)
...
3. 전사를 읽어라: $SRC (제목: \"$TITLE\", PLAUD id: $PLAUD_ID)
   frontmatter의 transcript 필드에는 이 경로를 그대로 적어라: $SRC
...
SLUG=$(printf '%s\n' "$OUT" | grep -m1 '^SLUG:' | sed 's/^SLUG:[[:space:]]*//' | iconv -c -f UTF-8 -t UTF-8 | tr -cd 'a-zA-Z0-9가-힣-')
SLUG=$(printf '%s' "$SLUG" | sed -E 's/^([0-9]{4}-[0-9]{2}-[0-9]{2}-)+//')  # 모델이 날짜를 넣으면 아래 TODAY와 중복
