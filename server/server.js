...
function findOutputVideoFile(basePath) {
  const dir = path.dirname(basePath);
  const base = path.basename(basePath, path.extname(basePath));
  const videoExts = ['.mp4', '.mkv', '.webm', '.mov'];
  try {
    const files = fs.readdirSync(dir);
    const found = files.find(f => f.startsWith(base) && videoExts.includes(path.extname(f).toLowerCase()));
...
function asciiFilenameFallback(filename, fallback) {
  const rawExt = path.extname(filename) || path.extname(fallback);
  const ext = /^[.\w-]{1,12}$/.test(rawExt) ? rawExt : path.extname(fallback);
  const fallbackBase = path.basename(fallback, path.extname(fallback)) || 'download';
...
function attachmentDisposition(filename, fallback = 'download.bin') {
  const safeFilename = path.basename(filename || fallback).replace(/[\\/]/g, '_') || fallback;
  const fallbackFilename = asciiFilenameFallback(safeFilename, fallback);
  return `attachment; filename="${fallbackFilename}"; filename*=UTF-8''${encodeHeaderFilename(safeFilename)}`;
}

function videoFormatAtMost(height) {
  return `bv[height<=${height}][ext=mp4]+ba[ext=m4a]/bv*[height<=${height}][ext=mp4]+ba[ext=m4a]/bv[height<=${height}]+ba/bv*[height<=${height}]+ba/b[height<=${height}]/b`;
}

const VIDEO_QUALITY_FORMATS = {
  best: 'bv[ext=mp4]+ba[ext=m4a]/bv*[ext=mp4]+ba[ext=m4a]/bv+ba/bv*+ba/b',
  '1080': videoFormatAtMost(1080),
  '720': videoFormatAtMost(720),
  '480': videoFormatAtMost(480),
  '360': videoFormatAtMost(360),
};

function buildVideoYtDlpArgs(url, outputPath, quality) {
  const fmt = VIDEO_QUALITY_FORMATS[quality] || VIDEO_QUALITY_FORMATS.best;
  return [
    ...COOKIES_ARGS,
    '-f', fmt,
...
const PLAUD_MAX_SECONDS = 5 * 60 * 60;
...
app.post('/download-video', (req, res) => {
  ...
  const tmpBase = path.join(tmpDir, `yt-mp4-${jobId}`);
  ...
  titleProc.on('close', () => {
    const safeName = (title || 'video').replace(/[^a-zA-Z0-9가-힣\s\-_]/g, '').trim().slice(0, 100);
    job.filename = `${safeName}.mp4`;
  });

  const args = buildVideoYtDlpArgs(url, `${tmpBase}.%(ext)s`, quality || 'best');
  ...
  runYtDlpWithCookieFallback(args, {
    label: `job:${jobId} video`,
    onFallback: () => {
      isAudioPhase = false;
    ...
    onStdout: (d) => {
      ...
      if (line.includes('Downloading video')) isAudioPhase = false;
      if (line.includes('Downloading audio')) isAudioPhase = true;

      const dlMatch = line.match(/\[download\]\s+([\d.]+)%/);
      if (dlMatch) {
        const pct = parseFloat(dlMatch[1]);
        if (!isAudioPhase) {
          job.progress = Math.min(pct * 0.75, 75);
          job.phase = `영상 다운로드 ${Math.round(pct)}%`;
        } else {
          job.progress = 75 + Math.min(pct * 0.1, 10);
          job.phase = `음성 다운로드 ${Math.round(pct)}%`;
        }
        return;
      }
      const fragMatch = line.match(/Fragment\s+(\d+)\s*\/\s*(\d+)/i);
      if (fragMatch) {
        ...
        job.progress = Math.min((cur / tot) * 75, 75);
      ...
      if (line.includes('[Merger]') || (line.includes('[ffmpeg]') && line.includes('Merging'))) {
        job.progress = 90;
        job.phase = '영상+음성 합치는 중...';
      }
    },
  }).then(() => {
    const actualFile = findOutputVideoFile(`${tmpBase}.mp4`);
    if (actualFile) {
      job.filePath = actualFile;
      const ext = path.extname(actualFile);
      if (ext && ext !== '.mp4') job.filename = job.filename.replace('.mp4', ext);
    ...
    console.error(`[job:${jobId}] error: ${err.stderr || err.message}`);
    job.status = 'error';
    job.phase = '다운로드 실패';
    job.error = (err.stderr || err.message).slice(0, 300);
  }).finally(() => {
...
app.post('/download-split', async (req, res) => {
  ...
  try {
    ...
    if (!fs.existsSync(actual)) throw new Error('다운로드된 파일을 찾을 수 없습니다');
    job.filePath = actual;
...
app.get('/file/:id/:part?', (req, res) => {
  ...
  if (job.type === 'split' && partNum) {
    ...
    res.setHeader('Content-Disposition', attachmentDisposition(file.filename, `audio-part${partNum}.mp3`));
  ...
  const mimeType = getMimeType(job.filePath);

  res.setHeader('Content-Type', mimeType);
  res.setHeader('Content-Disposition', attachmentDisposition(filename, `download${path.extname(job.filePath) || '.bin'}`));
